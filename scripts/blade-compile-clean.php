...
function marker(string $kind, string $phase, string $source, int $start, int $end, string $style): string
{
    ...
    $value = "blade-parser:{$kind}:{$phase}:{$startLine}:{$startColumn}:{$endLine}:{$endColumn}";
...
function propTypes(string $source): array
{
    ...
    foreach ($directives[0] as $directive) {
        ...
        $props = substr($source, $open + 1, $close - $open - 1);

        if (!preg_match_all('/([\'\"])([A-Za-z_]\w*)\1\s*=>\s*([^,\n]+)/', $props, $matches, PREG_SET_ORDER)) {
            continue;
        }
...
function wrapHtmlExpressions(string $compiled): string
{
    ...
    if (!preg_match_all('/\/\*\s*blade-parser:expression:(start|end):\d+:\d+:\d+:\d+\s*\*\//', $compiled, $matches, PREG_OFFSET_CAPTURE)) {
        if (preg_match_all('/\/\*\s*blade-parser:/', $compiled, $partialMatches)) {
            fwrite(STDERR, "wrapHtmlExpressions: found blade-parser comments but none matched expression pattern\n");
        }
...
try {
    ...
    fwrite(STDERR, 'blade-parser: prepareStringsForCompilationUsing reflection failed, markers disabled: ' . $e->getMessage() . "\n");
}
...
foreach ($items as $item) {
    try {
        ...
        foreach (propTypes($item['content']) as $name => $type) {
            $output .= "/** @var {$type} \${$name} */\n";
        }
        $output .= "?>\n" . $compiled;
